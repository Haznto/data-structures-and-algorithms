...
const getNames = (arr) => {
  return arr.map(element => Array.from(element.name).reduce((a,b) => b + a,'' ));
};
...
const appendFirstToLast = (arr) => {
  arr.map(element => element === arr[arr.length-1]? arr.push(arr[0]): element );
};
...
const addBirthYearProperty = (obj, year) => {
  let arr = [obj];
  arr.map(element => element.yearBorn = year);
};
