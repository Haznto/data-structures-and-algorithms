...
const screenForNames = (arr) => {
  const regex = /^(Mrs|Mr|Dr|Ms).\s[A-Z]{1}[a-z]{1,}/;
  const filteredArr = arr.filter((name) => regex.test(name));
  return filteredArr;
};
...
let biggerThanLuke = (arr) => {
  let filt = arr.filter(char => {
    if(char.mass >77) return char.name;
  });
  let strs = filt.map(element => element.name);
  return strs.join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  return winProbability.some(comb => {
    const [symbol1, symbol2, symbol3] = comb;
    return symbol1 !== '' && symbol1 === symbol2 && symbol1 === symbol3;
  });
};
