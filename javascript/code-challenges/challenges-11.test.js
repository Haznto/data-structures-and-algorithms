...
const count = (target, input) => {
  let counter = 0;
  input.map(arr => {
    let lengthofArr = arr.filter(element => element === target).length ;
    counter += lengthofArr;
    return lengthofArr;
  });
  return counter;
...
const divisibleByFiveTwoToThePower = (input) => {
  let dividableArrays = input.map(arr => {
    return arr.filter(element => typeof(element) === 'number' && element %5 === 0);
  });

  return dividableArrays.map( arrays => {
    return arrays.map(element => Math.pow(2,element));
  });
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(character => {
    if (character.name && (character.gender ==='female'|| character.gender ==='male') && data.indexOf(character) === data.length -1) {
      str += `${character.name}`;
    }
    else if (character.name && (character.gender ==='female'|| character.gender ==='male')) {
      str += `${character.name} and `;
    }
  });
